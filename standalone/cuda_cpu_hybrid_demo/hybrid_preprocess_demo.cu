...
static CsvData read_csv(const std::string &path) {
    ...
    if (std::getline(file, line)) {
        data.headers = split_csv_simple(line);
    }
...
static CsvData generate_data(int rows) {
    CsvData data;
    data.headers = {"age", "salary", "experience", "department", "city"};
    data.rows.reserve(rows);

    const char *departments[] = {"Engineering", "Finance", "HR", "Sales"};
    ...
    for (int i = 0; i < rows; i++) {
        ...
        row.push_back((i % 97 == 0) ? "NA" : std::to_string(age));
        row.push_back(std::to_string(salary));
        row.push_back(std::to_string(exp));
        row.push_back(departments[i % 4]);
        row.push_back(cities[i % 5]);
        data.rows.push_back(std::move(row));
    }
...
static std::vector<int> detect_numeric_columns(const CsvData &data) {
    std::vector<int> is_numeric_col(data.headers.size(), 1);
    ...
    for (int col = 0; col < (int)data.headers.size(); col++) {
        ...
        is_numeric_col[col] = (numeric_seen > 0 && text_seen == 0) ? 1 : 0;
    }
...
static std::vector<CpuColumnReport> process_categorical_cpu(
    const CsvData &data,
    const std::vector<int> &is_numeric_col,
    bool debug_threads,
    bool debug_all_threads
) {
    ...
    #pragma omp parallel
    {
        ...
        if (debug_threads && debug_all_threads) {
            #pragma omp critical
            {
                std::cout << "[CPU/OpenMP] thread " << thread_id
                          << " is active in categorical processing team of "
        ...
        for (int col = 0; col < (int)data.headers.size(); col++) {
            if (is_numeric_col[col]) {
                if (debug_threads && debug_all_threads) {
                    ...
                    {
                        std::cout << "[CPU/OpenMP] thread " << thread_id
                                  << " skipped numeric column " << col
                                  << " (" << data.headers[col] << ") because GPU handles it\n";
                    }
                }
                continue;
            }
            ...
            if (debug_threads) {
                #pragma omp critical
                {
                    std::cout << "[CPU/OpenMP] thread " << thread_id
                              ...
                              << " (" << data.headers[col] << "), rows 0-"
                              << (data.rows.empty() ? 0 : data.rows.size() - 1)
                              << "\n";
                }
            }
        }
...
__global__ void numeric_stats_kernel(
    ...
    if (debug_threads && (debug_limit < 0 || idx < debug_limit)) {
        printf("[GPU/CUDA stats] block=%d thread=%d global=%d -> column=%d row=%d value=%f\n",
...
__global__ void minmax_scale_kernel(
    ...
    if (debug_threads && (debug_limit < 0 || idx < debug_limit)) {
        printf("[GPU/CUDA scale] block=%d thread=%d global=%d -> column=%d row=%d before=%f\n",
               blockIdx.x, threadIdx.x, idx, col, row, value);
    }
...
static std::vector<GpuColumnReport> process_numeric_gpu(
    const CsvData &data,
    const std::vector<int> &numeric_cols,
...
int main(int argc, char **argv) {
    ...
    auto cpu_future = std::async(std::launch::async, [&]() {
        return process_categorical_cpu(data, numeric_flags, opts.debug_threads, opts.debug_all_threads);
    });

    float gpu_ms = 0.0f;
    auto gpu_reports = process_numeric_gpu(
    ...
    if (opts.debug_threads) {
        std::cout << "Thread debug mode: enabled, GPU print limit="
    ...
    std::cout << "\nTiming:\n";
    std::cout << "  GPU numeric kernel/copy time: " << std::fixed << std::setprecision(2) << gpu_ms << " ms\n";
    std::cout << "  CPU categorical wall time:   " << cpu_ms << " ms\n";
    std::cout << "  Total hybrid wall time:      " << total_ms << " ms\n";
    std::cout << "\nResult: CPU and GPU were both used in the same preprocessing run.\n";
