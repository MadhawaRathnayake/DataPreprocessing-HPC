...
// Analyze a single column (same logic as serial but called in parallel)
...
int analyzer_omp_analyze_dataset(char **data, char **headers, int num_rows, 
    ...
    printf("Starting OpenMP analysis with %d threads on %d rows x %d columns...\n", 
           num_threads > 0 ? num_threads : omp_get_max_threads(), num_rows, num_cols);
    
    // Parallel analysis of columns
    ...
    for (int col = 0; col < num_cols; col++) {
        ...
        #pragma omp critical
        {
            printf("Analyzed column %d/%d: %s (Thread %d)\n", 
                   col + 1, num_cols, headers[col], omp_get_thread_num());
        }
    }
    
    double end_time = omp_get_wtime();
    stats->processing_time = end_time - start_time;
    stats->num_threads = num_threads > 0 ? num_threads : omp_get_max_threads();
    
    printf("OpenMP analysis completed in %.4f seconds using %d threads.\n", 
           stats->processing_time, stats->num_threads);
