"""
MPI Pipeline Tab — 7-stage pipeline using MPI-parallelized C backend.
...
Backend: modules/analyzer_mpi/ → lib/libmpianalyzer.so
"""
...
# MPI pipeline stage modules
from pipeline_stages.mpi.stage_overview   import StageOverview
from pipeline_stages.mpi.stage_duplicates import StageDuplicates
from pipeline_stages.mpi.stage_missing    import StageMissing
from pipeline_stages.mpi.stage_outliers   import StageOutliers
from pipeline_stages.mpi.stage_scaling    import StageScaling
from pipeline_stages.mpi.stage_encoding   import StageEncoding
from pipeline_stages.mpi.stage_apply      import StageApply
...
class MPIPipelineTab(BaseTab):
    """MPI Parallel Processing tab — stepper/milestone pipeline UI."""
    ...
    def on_tab_selected(self):
        if self.app.csv_data:
            self.app.set_status("MPI Parallel Processing — configure stages, then Run Pipeline")
            stage = self._stages[self._current_stage]
            if hasattr(stage, "refresh"):
                stage.refresh()
            self._refresh_rail()
        else:
            self.app.set_status("MPI Parallel Processing — import a dataset first")
    ...
    def _build_left_rail(self):
        ...
        tk.Label(rail_outer, text="Pipeline Stages  [MPI]",
