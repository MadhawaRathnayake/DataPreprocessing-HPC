"""
Stage 7 — Apply & Preview  [OpenMP Parallel]
Executes all configured pipeline stages using OpenMP-parallelized C backend.
Backend: libompanalyzer.so (modules/analyzer_openmp/) via ctypes.

NOTE: Full OpenMP C integration is the next development phase.
Currently falls back to Python computation while the OpenMP C library is wired up.
"""
...
class StageApply:
    """Apply & Preview stage — runs the full pipeline using OpenMP backend."""
    ...
    def _build(self):
        ...
        ttk.Label(self.frame, text="Apply & Preview  [OpenMP Parallel]",
                  font=theme.FONT_TITLE, foreground=theme.HIGHLIGHT
                  ).pack(anchor="w", padx=14, pady=(12, 2))
        ttk.Label(self.frame,
                  text="Runs preprocessing pipeline using OpenMP-parallelized C backend (libompanalyzer.so).",
                  style="Muted.TLabel"
                  ).pack(anchor="w", padx=14, pady=(0, 2))

        # OpenMP info banner
        info = ttk.LabelFrame(self.frame, text="⚡ OpenMP Backend", padding=8)
        info.pack(fill="x", padx=14, pady=(0, 6))
        ttk.Label(info,
                  text=("Backend: modules/analyzer_openmp/ → libompanalyzer.so\n"
                        "Parallelism: #pragma omp parallel for (column-level, dynamic schedule)\n"
                        "Status: C library loaded — full ctypes integration is next phase."),
        ...
        self._run_btn = ttk.Button(btn_frame, text="▶  Run OpenMP Pipeline",
    ...
    def _run_pipeline(self):
        ...
        self.app.set_status("Running OpenMP parallel pipeline…")
    ...
    def _save_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            title="Save Processed Data (OpenMP)",
        )
        if not path:
            return
        try:
