...
class StageApply:
    ...
    def _run_pipeline(self):
        ...
        self.app.set_status("Running MPI parallel pipeline…")
        self.app.root.update()

        try:
            t_start = time.perf_counter()

            # ── TODO: Call MPI C backend ──────────────────────────────────────
            # When libmpianalyzer.so is fully wired, replace the Python logic
            # below with:
            #
            #   if self.app.mpi_lib:
            #       stats_ptr = self.app.mpi_lib.analyzer_mpi_create_stats(num_cols)
            #       self.app.mpi_lib.analyzer_mpi_analyze_dataset(
            #           data_ptr, headers_ptr, num_rows, num_cols, stats_ptr)
            #       json_bytes = self.app.mpi_lib.analyzer_mpi_get_stats_json(stats_ptr)
            #       # parse json_bytes and apply transformations
            #       self.app.mpi_lib.analyzer_mpi_free_stats(stats_ptr)
            # ─────────────────────────────────────────────────────────────────
    ...
    def _apply_duplicates(data, headers, cfg):
        ...
        for i, row in enumerate(data):
            ...
            if key not in seen:
                seen[key] = i; result.append(row)
            else:
                if keep == "last":
                    result.remove(data[seen[key]]); seen[key] = i; result.append(row)
                elif keep == "none":
    ...
    def _apply_missing(data, headers, cfg):
        ...
        for j, col in enumerate(headers):
            ...
            if (null_c / total * 100 if total else 0) <= threshold:
                keep_idxs.append(j)
        ...
        if mode == "apply_all":
            ...
            strategy_for = {col: col_cfg.get(col, {}).get("strategy", "Drop row") for col in headers}
        col_stats = {}
        for j, col in enumerate(headers):
            vals = []
            for row in data:
            ...
            nv = [v for v in vals if isinstance(v, float)]
            sv = [v for v in vals if isinstance(v, str)]
            col_stats[col] = {
                "mean":   sum(nv) / len(nv) if nv else 0,
                "median": sorted(nv)[len(nv) // 2] if nv else 0,
                "mode":   collections.Counter(sv or [str(v) for v in nv]).most_common(1)[0][0]
                          if (sv or nv) else "",
            }
        result = []
        for row in data:
            new_row = list(row); drop = False
            for j, col in enumerate(headers):
                if j >= len(new_row): continue
                val = new_row[j]
                if not val or not val.strip():
                    ...
                    if strat == "Drop row": drop = True; break
                    elif strat == "Mean":   new_row[j] = str(round(col_stats[col]["mean"], 4))
                    elif strat == "Median": new_row[j] = str(round(col_stats[col]["median"], 4))
                    elif strat == "Mode":   new_row[j] = col_stats[col]["mode"]
                    elif strat == "Fill constant": new_row[j] = fill_val or "0"
                    elif strat == "Forward-fill":
                        last = next((result[-k-1][j] for k in range(len(result))
                                     if j < len(result[-k-1]) and result[-k-1][j]), None)
                        new_row[j] = last or ""
            if not drop: result.append(new_row)
        return result, headers

    @staticmethod
    def _apply_outliers(data, headers, cfg):
        treatment = cfg.get("treatment", "skip")
        if treatment == "skip": return data, headers
        method = cfg.get("method", "iqr"); iqr_m = cfg.get("iqr_mult", 1.5)
        z_thr  = cfg.get("zscore_thr", 3.0); columns = cfg.get("columns", [])
        if not columns: return data, headers
        col_bounds = {}
        for col in columns:
        ...
        if treatment == "flag":
            headers = list(headers) + [f"{c}_outlier" for c in col_bounds]
        result = []
        for row in data:
            new_row = list(row); drop = False; flags = {}
            for col, (lo, hi) in col_bounds.items():
                j = headers.index(col) if col in headers else -1
                if j == -1 or j >= len(new_row): flags[col] = "0"; continue
                try: val = float(new_row[j]); is_out = val < lo or val > hi
                except ValueError: is_out = False
                if is_out:
                    if treatment == "remove": drop = True; break
                    elif treatment == "cap":
                        new_row[j] = str(round(min(max(float(new_row[j]), lo), hi), 6))
                flags[col] = "1" if is_out else "0"
            if not drop:
                if treatment == "flag": new_row += [flags.get(c,"0") for c in col_bounds]
    ...
    def _apply_scaling(data, headers, cfg):
        ...
        if method == "skip": return data, headers
        ...
        for col in columns:
            ...
            if method == "minmax":
                ...
                sv = sorted(vals); n = len(sv)
                med = sv[n//2]; q1 = sv[n//4]; q3 = sv[3*n//4]; iqr = (q3-q1) or 1
                for row in data:
                    if j < len(row) and row[j] and row[j].strip():
    ...
    def _apply_encoding(data, headers, cfg):
        methods = cfg.get("column_methods", {}); drop_orig = cfg.get("drop_original", True)
        if not methods: return data, headers
        new_headers = list(headers); col_insertions = []
        for col, method in methods.items():
            if method == "Skip" or col not in new_headers: continue
            ...
            if method == "Label encode":
                ...
                for row in data:
                    if j < len(row): row[j] = label_map.get(row[j], row[j])
            elif method == "One-hot encode":
                unique_vals = sorted(set(row[j] for row in data if j < len(row) and row[j]))
                col_insertions.append((j, [f"{col}_{v}" for v in unique_vals], unique_vals, col))
        for j, extra_cols, unique_vals, col in reversed(col_insertions):
            for row in data:
                val = row[j] if j < len(row) else ""
                ohe = ["1" if val == uv else "0" for uv in unique_vals]
                if drop_orig: row[j:j+1] = ohe
                else: row[j+1:j+1] = ohe
