"""
OpenMP Pipeline Tab — 7-stage pipeline using OpenMP-parallelized C backend.
...
Backend: modules/analyzer_openmp/ → lib/libompanalyzer.so
"""
...
# OpenMP pipeline stage modules
from pipeline_stages.openmp.stage_overview   import StageOverview
from pipeline_stages.openmp.stage_duplicates import StageDuplicates
from pipeline_stages.openmp.stage_missing    import StageMissing
from pipeline_stages.openmp.stage_outliers   import StageOutliers
from pipeline_stages.openmp.stage_scaling    import StageScaling
from pipeline_stages.openmp.stage_encoding   import StageEncoding
from pipeline_stages.openmp.stage_apply      import StageApply
...
class OpenMPPipelineTab(BaseTab):
    """OpenMP Parallel Processing tab — stepper/milestone pipeline UI."""
    ...
    def on_tab_selected(self):
        if self.app.csv_data:
            self.app.set_status("OpenMP Parallel Processing — configure stages, then Run Pipeline")
            stage = self._stages[self._current_stage]
            if hasattr(stage, "refresh"):
                stage.refresh()
            self._refresh_rail()
        else:
            self.app.set_status("OpenMP Parallel Processing — import a dataset first")
    ...
    def _build_left_rail(self):
        ...
        tk.Label(rail_outer, text="Pipeline Stages  [OpenMP]",
